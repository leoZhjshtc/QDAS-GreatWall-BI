 ...
 <body>
    ...
    <div id="cc" class="easyui-layout" style="width:100%;height:100%;">
	    <div data-options="region:'north',collapsible:false" style="height:56px;padding:5px;overflow: hidden">
			...
			<form style="float: right;height: 100%">
		    	...
		    	<select id="jgtime" style="width:80px;padding-left: 10px">
					...
					<input id="qbSwitch" style="width:200px;height:30px">
					开始时间：<input id="qbShowStartTime" data-options="" style="width:150px">
					结束时间：<input id="qbShowEndTime" data-options="" style="width:150px">
	    	</form>
	    ...
	    <div data-options="region:'west',collapsible:false" style="width:30%;padding: 10px">
	    	<form id="dataForm" method="post">
	    		<div style="margin-top:15px">
	                <input class="easyui-textbox" name="TEERZEUGNIS" style="width:100%;padding-left: 20px" data-options="label:'<font>产线名:</font>',editable:false">
	            </div>
	            <div style="margin-top:15px">
	                <input class="easyui-textbox" name="TETEILNR" style="width:100%;padding-left: 20px" data-options="label:'<font>零件号:</font>',editable:false">
	            </div>
	            <div style="margin-top:15px">
	                <input class="easyui-textbox" name="TEBEZEICH" style="width:100%;padding-left: 20px" data-options="label:'<font>零件名:</font>',editable:false">
	            </div>
	            <div style="margin-top:15px">
	        ...
	        <input type="hidden" id="lbK1007Num" name="lbK1007Num" value="${k1007Num }">
	    </div>
	...
	<script type="text/javascript">
		...
		var qbRadioIndex=1;
		$(function(){
			$('#qbShowStartTime').datetimebox({
				editable:false
			})
			$('#qbShowEndTime').datetimebox({
				editable:false
			})
		...
		function getFormData(){
			...
			var qbShowEndTime='';
			var productLineName='';
			var searchRowsFlag=$('#qbSwitch').switchbutton('options').checked;
			if(searchRowsFlag){
				qbShowStartTime=$('#qbShowStartTime').datetimebox('getValue');
				qbShowEndTime=$('#qbShowEndTime').datetimebox('getValue');
			}
			if($('#index').val()=='1'){
				productLineName=$('#searchPlName').textbox('getValue');
			}else{
			...
			$.ajax({
				...
				data:{
					arrIndex:index,
					searchRowsFlag:searchRowsFlag,
					startTime:qbShowStartTime,
					endTime:qbShowEndTime,
					teilId:$('#teilId').val(),
					productLineName:productLineName,
					index:$('#index').val(),
					k1007Num:$('#lbK1007Num').val()
				},
				url:'<%=basePath%>qb/getQbForm',
				success:function(data){
					$('#dataForm').form('clear');
					//$('#titleForm').form('clear');
					$('#qbDig').panel('setTitle','质量看板   '+index+'/'+data.total)
					if(data.total==index){
						index=1;
					}else{
						index += 1;
		...
		function initChart(data,lineBar){
			...
			if(lineBar=='0'){
				...
				for(var i=0;i<rows.length;i++){
					...
					tooltipPMBEZ.push(rows[i].MABEZ);
				}
				qbShowCharts=initLineChart2('qbCharts',xValue,yValue,upLimit,downLimit,mData,tooltipTime,tooltipPMBEZ);
			}else if(lineBar=='1'){
