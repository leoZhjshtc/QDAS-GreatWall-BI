...
function initLineChart2(divId,xValues,yValues,upLimitData,downLimitData,mData,tooltipTime,tooltipPMBEZ,tooltipMABEZ,tooltipK0014,tooltipNEST){
	var upanddown;
	...
	option = {
			...
			title : {
		        text: '上公差限：'+upLimitData+' | 下公差限：'+downLimitData,
		        x: 'center',
		        align: 'right'
		    },
		    grid: {
			 ...
			 tooltip: {
	            ...
	            formatter:function(params){
	            	//console.log(params);
	            	var res='<div><p>测量值：'+params[0].data+'</p><p>测量时间：'+tooltipTime[params[0].dataIndex]+'</p><p>测量设备：'+tooltipPMBEZ[params[0].dataIndex]+'</p><p>加工设备：'+tooltipMABEZ[params[0].dataIndex]+'</p><p>产品号：'+tooltipK0014[params[0].dataIndex]+'</p><p>主轴号：'+tooltipNEST[params[0].dataIndex]+'</p></div>';
	            	return res;
	            }
	        },
		    xAxis: {
